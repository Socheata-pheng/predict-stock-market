...
st.set_page_config(layout="wide", page_title="Forcastor")

# Sidebar
st.sidebar.markdown("<h1 style='text-align: center; font-size: 30px;'><b>Forcastor</b></h1>", unsafe_allow_html=True)
...
st.markdown("<p style='text-align: center;'>This is a simple web app for stock price prediction using the <a href='https://facebook.github.io/prophet/'>Prophet</a> library.</p>", unsafe_allow_html=True)
...
if selected_tab == "Dataframes":
    ...
    st.markdown("<h2><span style='color: red;'>{}</span> Historical Data</h2>".format(selected_stock), unsafe_allow_html=True)
    ...
    st.markdown("<h2><span style='color: red;'>{}</span> Forecast Data</h2>".format(selected_stock), unsafe_allow_html=True)
...
if selected_tab == "Statistics":
    st.markdown("<h2><span style='color: red;'>Descriptive </span>Statistics</h2>", unsafe_allow_html=True)
...
if selected_tab == "Forecasting":
    # Plotting forecast
    st.markdown("<h2><span style='color: red;'>{}</span> Forecast Plot</h2>".format(selected_stock), unsafe_allow_html=True)
    ...
    st.markdown("<h2><span style='color: red;'>{}</span> Forecast Components</h2>".format(selected_stock), unsafe_allow_html=True)
...
if selected_tab == "Comparison":
    if selected_stocks:
        ...
        st.markdown("<h2><span style='color: red;'>{}</span> Forecast Comparison Plot</h2>".format(', '.join(selected_stocks)), unsafe_allow_html=True)
        ...
        for i, data in enumerate(stocks_data):
            if data is not None:
                ...
                forecast = model.predict(future)
                forecast = forecast[forecast['ds'] >= end_date_datetime]
                st.markdown("<h3><span style='color: red;'>{}</span> Forecast DataFrame</h3>".format(selected_stocks[i]), unsafe_allow_html=True)
